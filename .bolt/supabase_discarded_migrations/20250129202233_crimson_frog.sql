/*
  # Add channels and private messaging support
  
  1. New Tables
    - channels: For group messaging
    - channel_members: For managing channel membership
  
  2. Changes to messages
    - Add channel_id and recipient_id
    - Update constraints and policies
  
  3. Security
    - RLS enabled on all tables
    - Policies for public/private channels
    - Updated message access policies
*/
...
SELECT 
  ...
  'General',
  'Default channel for all messages',
  false,
  (SELECT id FROM auth.users ORDER BY created_at ASC LIMIT 1)
WHERE NOT EXISTS (
  SELECT 1 FROM channels WHERE id = '00000000-0000-0000-0000-000000000000'::uuid
);
...
-- Move existing messages to default channel
UPDATE messages 
SET channel_id = '00000000-0000-0000-0000-000000000000'::uuid
WHERE channel_id IS NULL AND recipient_id IS NULL;
