...
-- Create improved account statistics function with input parameter name matching the call
CREATE OR REPLACE FUNCTION get_account_statistics(input_account_id UUID)
RETURNS JSON AS $$
DECLARE
  account_stats JSON;
BEGIN
  WITH trade_stats AS (
    SELECT
      ...
      calculate_account_drawdown(input_account_id) as max_drawdown,
      COALESCE(
    ...
    FROM trades t
    WHERE t.account_id = input_account_id
    AND t.status = 'closed'
  )
